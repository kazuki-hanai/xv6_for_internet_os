...
kvminit()
{
	...
	kvmmap((uint64_t)etext, (uint64_t)etext, PHYSTOP-(uint64_t)etext, PTE_R | PTE_W);
...
// physical memory.
void
uvmunmap(pagetable_t pagetable, uint64_t va, uint64_t size, int do_free)
{
	uint64_t a, last;
	pte_t *pte;
	uint64_t pa;
...
uint64_t
uvmalloc(pagetable_t pagetable, uint64_t oldsz, uint64_t newsz)
{
	char *mem;
	uint64_t a;
...
uvmdealloc(pagetable_t pagetable, uint64_t oldsz, uint64_t newsz)
{
	...
	uint64_t newup = PGROUNDUP(newsz);
